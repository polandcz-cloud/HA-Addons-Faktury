...
async function main() {
  ...
  app.get('/api/ares/:ico', async (req, res) => {
    ...
    if (!ico || ico.length < 7) return res.json({ error: 'Invalid ICO' });
    
    try {
      const https = require('https');
      const url = `https://ares.gov.cz/ekonomicke-subjekty-v-registru-ekonomickych-subjektu/rest/ekonomicke-subjekty/${ico}`;
      ...
      const data = await new Promise((resolve, reject) => {
        https.get(url, { headers: { 'Accept': 'application/json' } }, (resp) => {
          ...
          resp.on('end', () => {
            if (resp.statusCode !== 200) return reject(new Error('Not found'));
      ...
      const street = [addr.nazevUlice, addr.cisloDomovni ? (addr.cisloDomovni + (addr.cisloOrientacni ? '/' + addr.cisloOrientacni : '')) : ''].filter(Boolean).join(' ');
      ...
      console.error('[ARES] Error for ICO', ico, ':', err.message);
  ...
  app.put('/api/supplier', (req, res) => {
    const s = req.body;
    run(`UPDATE supplier SET name=?,ico=?,dic=?,street=?,city=?,zip=?,bank_account=?,bank_code=?,iban=?,swift=?,email=?,phone=?,web=?,is_vat_payer=?,number_format=?,quote_number_format=?,default_due_days=?,default_quote_validity_days=?,default_note=?,default_quote_intro=?,default_quote_outro=?,currency=?,logo_image=?,logo_text=? WHERE id=1`,
